#include "/settings/color/netherColorSettings.glsl"
...
vec4 netherBasalt  = vec4(vec3(NETHER_BR, NETHER_BG, NETHER_BB) / 255.0, 1.0) * NETHER_BI;
    ...
    vec4 netherColSqrt = vec4(normalize(fogColor + 0.0001), length(fogColor));
        ...
        vec4 netherColSqrt = mix(netherNether, (netherValley * isValley + netherCrimson * isCrimson +
            netherWarped * isWarped + netherBasalt * isBasalt) / max(nBiomeWeight, 0.0001), nBiomeWeight);
    #else
